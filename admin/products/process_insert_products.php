...
$types_name = !empty($_POST['types_name']) ? explode(',', $_POST['types_name']) : array();
$folder = 'images/';
...
if (empty($error)) {
	...
	//insert hashtags (only if types_name is not empty)
	if (!empty($types_name) && count($types_name) > 0) {
		foreach ($types_name as $type_name) {
			$type_name = trim($type_name); // Remove whitespace
			if (!empty($type_name)) {
