...
if ( isset($_GET['days']) ) {
	...
	$sql_command_select = "
		SELECT sum(total_price) as 'cash_each_day', DATE_FORMAT(order_time, '%d-%m') as 'day'
		FROM receipts
	...
	if ( $max_day_of_this_month_to_get > $day_today ) {
		...
		for ( $i = $start_day_of_last_month; $i <= $max_day_of_previous_month; $i++ ) {
			$index = sprintf('%02d-%s', $i, $previous_month);
			$array[$index] = 0;
		}
	...
	for ( $i = $start_day_of_this_month; $i <= $day_today; $i++ ) {
		$index = sprintf('%02d-%s', $i, $this_month);
		$array[$index] = 0;
	}
...
if ( isset($_GET['year']) ) {	
	$year = $_GET['year'];
	$sql_command_select = "
		SELECT sum(total_price) as 'cash_each_day',month(order_time) as 'month'
		FROM receipts
...
if ( isset($_GET['month']) ) {
	...
	for ( $i = 1; $i <= $max_day_of_this_month_to_get; $i++ ) {
		$index = sprintf('%02d-%s', $i, $that_month);
		$array[$index] = 0;
	}
	foreach ($query_sql_command_select as $array_cash_each_day ) {
		$array[$array_cash_each_day['day']] = (float)$array_cash_each_day['cash_each_day'];
